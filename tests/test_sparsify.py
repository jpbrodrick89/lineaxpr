...
def test_diagonal_seed_scales_per_column():
    ...
    out_identity = _to_dense(sparsify(lin)(Identity(n, dtype=jnp.float64)))
    out_diag = _to_dense(sparsify(lin)(Diagonal(v)))
...
def test_constant_hessian_folds_to_literal():
    ...
    def extract(y):
        ...
        return _to_dense(sparsify(hvp)(Identity(y.size, dtype=y.dtype)))
...
def test_nested_jit_inside_linear_fn():
    ...
    y = jnp.zeros(20)
    _, hvp = jax.linearize(jax.grad(f), y)
    out = _to_dense(sparsify(hvp)(Identity(20, dtype=y.dtype)))
